...
if [ "$MODE" = "check" ]; then
  if [ "$PIN_OK" -eq 1 ]; then
    ...
    echo "agentic-context $LATEST is available but outside your pin '$PIN'. See MIGRATIONS.md, then use --force."
  fi
  [ "$QUIET" -eq 1 ] && exit 0
  report_local_state
  exit 0
...
if [ "$PIN_OK" -ne 1 ]; then
  ...
  echo "This is a major upgrade. Read MIGRATIONS.md, then re-run with --force." >&2
  exit 1
fi
...
[ -f "$SRC/core/.context/index.md" ] && cp "$SRC/core/.context/index.md" "$CONTEXT_DIR/index.md"
...
if [ -f "$AGENTS_FILE" ] && [ -f "$SRC/core/AGENTS.md" ]; then
  if grep -q '^<!-- agentic-context:begin' "$AGENTS_FILE" 2>/dev/null; then
    ...
    echo "  AGENTS.md: no managed block found — left untouched. Merge manually if required."
  fi
fi
