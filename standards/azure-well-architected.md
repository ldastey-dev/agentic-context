...
| **Auto-scaling** | Configure elasticity per compute model in use. **AKS:** Cluster autoscaler + Horizontal Pod Autoscaler (CPU/memory) or KEDA (event-driven); containers must declare `requests` and `limits` — HPA cannot function without them. **Container Apps:** Replica scaling rules (HTTP concurrency, CPU/memory, KEDA scalers); define min and max replicas. **App Service:** Autoscale rules on the App Service Plan (instance count by CPU/memory/queue length). **Azure Functions:** Consumption or Flex Consumption for event-driven scale; Premium plan for predictable warm instances and VNet integration; set per-function concurrency limits. Always bound min/max replicas or instances to control cost and protect baseline availability. |
